...
namespace ptoVenta
{
    class Ticket
    {
        ...
        ArrayList subtitleLines = new ArrayList();
        ...
        public void AddsubTitleLine(string line)
        {
            titleLines.Add(line);
        }
        ...
        private void DrawImage()
        {
            if (headerImage != null)
            {
                try
                {
                    ...
                    gfx.DrawImage(headerImage, 0, 0, 30, 30);
                }
                catch (Exception)
                {
                }
        ...
        private void DrawHeader()
        {
            ...
            {
                printFont = new Font(fontName, 14, FontStyle.Regular);
        ...
        private void DrawTitle()
        {
            foreach (string title in titleLines)
            {
                ...
                {
                    printFont = new Font(fontName, 12, FontStyle.Regular);
                    int currentChar = 0;
                    int titleLenght = title.Length;

                    while (titleLenght > maxChar)
                    {
                        line = title.Substring(currentChar, maxChar);
                        gfx.DrawString(line, printFont, myBrush, leftMargin, YPosition(), new StringFormat());
        ...
        private void DrawsubTitle()
        {
            foreach (string subtitle in subtitleLines)
            {
                ...
                {
                    int currentChar = 0;
                    int subtitleLenght = subtitle.Length;

                    while (subtitleLenght > maxChar)
                    {
                        line = subtitle.Substring(currentChar, maxChar);
                        gfx.DrawString(line, printFont, myBrush, leftMargin, YPosition(), new StringFormat());

                        count++;
                        currentChar += maxChar;
                        subtitleLenght -= maxChar;
                    }
                    line = subtitle;
                    gfx.DrawString(line.Substring(currentChar, line.Length - currentChar), printFont, myBrush, leftMargin, YPosition(), new StringFormat());
                    count++;
        ...
        private void DrawItems()
        {
            ...
            //gfx.DrawString("DESCRIPCION              MONTO", printFont, myBrush, leftMargin, YPosition(), new StringFormat());
        ...
        private void DrawFooter()
        {
            foreach (string footer in footerLines)
            {
                if (footer.Length > maxChar)
                {
                    ...
                    line = footer;
                    //gfx.DrawString(line.Substring(currentChar, line.Length - currentChar), printFont, myBrush, leftMargin, YPosition(), new StringFormat());
                    float XPosition = maxChar - footer.Length / 2;
                    gfx.DrawString(line.Substring(currentChar, line.Length - currentChar), printFont, myBrush, XPosition, YPosition(), new StringFormat());
                    count++;
                }
                else
                {
